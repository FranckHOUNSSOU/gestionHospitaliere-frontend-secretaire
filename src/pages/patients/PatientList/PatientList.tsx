...
import { Search, Plus, Eye, CreditCard as Edit2, Download, Users, Loader2 } from 'lucide-react';
...
export default function PatientList() {
  ...
  useEffect(() => {
    getPatient.findAll()
      .then((data: any[]) => setAllPatients(data as any))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  const filtered = (allPatients as any[]).filter((p: any) => {
    const q = search.toLowerCase();
    const nom    = (p.nom    ?? '').toLowerCase();
    const prenom = (p.prenom ?? '').toLowerCase();
    const tel    = (p.telephoneMobile ?? p.telephone ?? '');
    const ipp    = (p.numeroIpp ?? '').toLowerCase();
    const matchSearch  = nom.includes(q) || prenom.includes(q) || tel.includes(q) || ipp.includes(q);
    const matchStatus  = statusFilter === 'all' || (p.statutProfil ?? p.status) === statusFilter;
    const matchGender  = genderFilter === 'all' || (p.sexe ?? p.genre ?? p.gender) === genderFilter;
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'var(--c-accent-bg)', border: '1px solid var(--c-accent-bd)', borderRadius: '8px', padding: '6px 12px' }}>
          ...
          <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--c-accent)' }}>
            {loading ? '…' : `${allPatients.length} patients enregistrés`}
          </span>
        </div>
      ...
      <div className="adm-card">
        <div className="adm-card-head" style={{ gap: '10px', flexWrap: 'wrap' }}>
          <div className="adm-search" style={{ flex: 1, minWidth: '200px' }}>
            ...
            <input
              type="text"
              placeholder="Rechercher par nom, téléphone, IPP..."
              value={search}
            ...
            <table className="adm-table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((p: any) => {
                    const statut = p.statutProfil ?? p.status ?? 'active';
                    const { variant, label } = statusBadge(statut);
                    const groupe = p.groupeSanguin ?? p.bloodType ?? '—';
                    const sexe   = p.sexe ?? p.genre ?? p.gender;
                    const dob    = p.dateNaissance ?? p.dateOfBirth;
                    const tel    = p.telephoneMobile ?? p.telephone ?? '—';
                    const ipp    = p.numeroIpp ?? '—';
                    const initP  = (p.prenom ?? p.firstName ?? '?')[0] ?? '?';
                    const initN  = (p.nom    ?? p.lastName  ?? '?')[0] ?? '?';
                    return (
                      <tr key={p.id}>
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            ...
                            <div>
                              <p className="adm-cell-name">
                                {p.nom ?? p.lastName} {p.prenom ?? p.firstName}
                              </p>
                              <p className="adm-cell-mono">Groupe {groupe}</p>
                            </div>
                          </div>
                        </td>
                        <td>
                          <span style={{ fontWeight: 600, color: 'var(--c-t0)', fontSize: '12.5px' }}>
                            {dob ? `${calcAge(dob)} ans` : '—'}
                          </span>
                          <span style={{ color: 'var(--c-t3)', fontSize: '12px', marginLeft: '4px' }}>
                            {sexe === 'M' ? 'H' : sexe === 'F' ? 'F' : '—'}
                          </span>
                        </td>
                        <td><span className="adm-cell-mono" style={{ fontSize: '12px' }}>{tel}</span></td>
                        <td><span className="adm-cell-mono">{ipp}</span></td>
                        <td>
                          <span className="adm-cell-mono">
                            {p.createdAt ?? p.registrationDate
                              ? new Date(p.createdAt ?? p.registrationDate).toLocaleDateString('fr-FR')
                              : '—'}
                          </span>
                        </td>
